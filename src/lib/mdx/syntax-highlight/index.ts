...
export function syntaxHighlight() {
  return async (tree: Node) => {
    ...
    async function visitor(node: Element, index: number, parent: Element) {
      ...
      let lightTheme = (await import('shiki/dist/themes/github-light.mjs'))
        .default
      lightTheme = {
        ...lightTheme,
        colors: {
      ...
      let darkTheme = (await import('shiki/dist/themes/github-dark.mjs'))
        .default
      darkTheme = {
        ...darkTheme,
        colors: {
...
function getNodeLanguage(node: Element): string | undefined {
  ...
  const language = node.properties.className.find((d) => {
