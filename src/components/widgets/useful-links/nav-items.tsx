  ...
  return (
    <div>
      ...
      <nav className="mt-4 flex flex-col space-y-2 text-sm">
        {navItems.map(link => (
          <Link showAnchorIcon={link.external === true} isBlock key={link.label} className="text-sm text-palettes-primary-25 transition-colors duration-300 hover:text-palettes-primary-40 dark:text-palettes-primary-90 dark:hover:text-primary-700" href={link.href} target={link.target}>
            {link.label}
          </Link>
        ))}
      </nav>
